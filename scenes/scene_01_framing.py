...
Companion to the Introduction of "Moral AI". All on-screen text is paraphrase,
pulled from the active language module (English / Italian) via content.D.
Render: MORALAI_LANG=en|it manim -qm --disable_caching scenes/scene_01_framing.py Framing
"""
import os, sys
...
class Framing(Scene):
    def construct(self):
        ...
        # --- Act 1: the alarm (our paraphrase, no quotation marks) --------
        alarm = body(F["alarm"], color=INK, size=38, w=30)
        alarm.move_to([0, 0.4, 0])
        sub = caption(S["alarm_sub"], color=MUTE, size=24)
        ...
        self.play(FadeIn(ph, shift=DOWN * 0.2), FadeIn(oh, shift=DOWN * 0.2), run_time=0.8)
        ...
        self.play(LaggedStart(*[FadeIn(b, shift=LEFT * 0.15) for b in ob],
                              lag_ratio=0.25, run_time=1.8))
        self.wait(1.6)
        self.play(FadeOut(VGroup(divider, ph, pb, oh, ob)), run_time=0.7)

        # --- Act 3: the glass is both -------------------------------------
        q = caption(F["verdict_question"], color=MUTE, size=26)
        q.move_to([0, 2.0, 0])
        ...
        empty_lbl = caption(S["glass_empty"], color=BAD, size=22).next_to(glass, RIGHT, buff=0.9).shift(UP * 0.85)
        full_lbl = caption(S["glass_full"], color=GOOD, size=22).next_to(glass, LEFT, buff=0.9).shift(DOWN * 0.6)
        ...
        # verdict (our framing of the authors' stance, not a quotation)
        pre = body(F["verdict_pre"] + "  ", color=INK, size=38)
        both = Text(F["verdict"], font_size=44, weight=BOLD)
        both.set_color_by_gradient(GOOD, BAD)
        verdict = VGroup(pre, both).arrange(RIGHT, buff=0.12)
        verdict.move_to([0, -2.2, 0])
        gloss = caption(F["verdict_gloss"], color=MUTE, size=18, w=82)
